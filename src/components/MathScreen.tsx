      ...
      <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full h-full">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-4 flex flex-col items-center">
          {!store.isLifelineUsed ? (
            ...
            <div className="w-full mt-4 flex justify-center">
              ...
              {/* ARCHITECT FIX: Added dir="ltr" to the main flex container so Group 1 is on the left */}
              {operator === '+' && (
        ...
        <div className="shrink-0 bg-white border-t-2 border-gray-100 p-4 sm:p-6 z-20">
          <div className="max-w-sm mx-auto flex flex-col gap-3">
            <input type="text" inputMode="numeric" pattern="[0-9]*" value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={store.interactionState === 'success'} placeholder="התשובה היא..." className="w-full text-center text-4xl sm:text-5xl font-black text-purple-700 bg-purple-50 border-4 border-purple-300 rounded-2xl py-3 focus:outline-none" />
            <button onClick={handleSubmit} disabled={!inputValue || store.interactionState === 'success'} className="w-full py-4 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-300 text-white font-bold text-2xl rounded-2xl shadow-lg transition-transform active:scale-95">בדוק אותי!</button>
          </div>
        </div>
